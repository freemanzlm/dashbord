...
public class ListingService extends BaseService {
	private CommonLogger logger = CommonLogger
			.getInstance(ListingService.class);
	@Autowired
	...
	private String url(String url) {
		...
		return secureUrl(ResourceProvider.ListingRes.listingBase) + url;
	}

	@SuppressWarnings("unchecked")
	public Boolean confirmListings(SelectableListing[] listings,
			String promoId, Long uid) throws PromoException {
		...
		try {
			...
			if (null != resp) {
				UploadListingResponse<Listing> respEntity = resp
						.getEntity(type);
				if (null != respEntity
						&& respEntity.getAckValue() == AckValue.SUCCESS) {
					return Boolean.TRUE;
				}
			}
		} catch (Throwable e) {
	...
	/**
	 * Get seller applied sku listings in this promotion. These listings are
	 * uploaded by seller.
	 * 
	 * @param promoId
	 * @param uid
	 ...
	 */
	public <T> List<T> getListingsByPromotionId(String promoId, Long uid,
			boolean isUploaded) throws PromoException {
		String uri = "";
		...
		if (isUploaded)
			...
			uri = url(params(ResourceProvider.ListingRes.getPromotionListings,
					new Object[] { "{promoId}", promoId, "{uid}", uid }));
		// type = new GenericType<ListDataServiceResponse<Map<String,
		// Object>>>(){};
		...
		if (Status.OK.getStatusCode() == resp.getStatus()) {
			@SuppressWarnings("unchecked")
			ListDataServiceResponse<T> data = (ListDataServiceResponse<T>) resp
					.getEntity(type);
			...
			throw new PromoException(
					"Failed to retrieve the SKU listing list with provided promo ID: "
							+ promoId + ", user ID: " + uid);
		}
		return null;
	}
	 ...
	 * @throws PromoException
	 */
	public boolean uploadListings(List<Listing> uploadListings, String promoId,
			Long uid) throws PromoException {
			...
			if (null != general) {
				if (AckValue.SUCCESS == general.getAckValue()) {
					...
					if (errorCode == ErrorType.DateExpiredException.getCode()) {
						throw new PromoException(
								ErrorType.DateExpiredException,
								"ACTION1_END_DATE");
					}
				}
			}
		...
		throw new PromoException(ErrorType.UnableUploadDealsListing,
				Status.fromStatusCode(resp.getStatus()));
	}
	 ...
	 * @throws PromoException
	 */
	public boolean submitListings(String promoId, Long uid)
			throws PromoException {
		...
		if (Status.OK.getStatusCode() == resp.getStatus()) {
			...
			throw new PromoException(ErrorType.UnableSubmitDealsListing,
					Status.fromStatusCode(resp.getStatus()));
		}
	}
	 ...
	 * @throws IOException
	 */
	public String uploadListingAttachment(String skuId, String promoId,
			Long userId, String key, final MultipartFile uploadFile,
		...
		if (Status.OK.getStatusCode() == resp.getStatus()) {
			...
			if (null != general) {
				if (AckValue.SUCCESS == general.getAckValue()) {
					return params(
							ResourceProvider.ListingRes.listingAttachment,
							new Object[] { "{promoId}", promoId, "{userId}",
									userId, "{skuId}", skuId, "{key}", key });
				} else {
					return null;
				}
			} else {
			...
			throw new Exception(
					getMessage("attachment.validation.message.notcorrecttype"));
		}
	}
	 ...
	 * @throws PromoException
	 */
	public ListingAttachment downloadListingAttachment(String promoId,
			Long userId, String skuId, String key) throws PromoException {
		String url = url(params(ResourceProvider.ListingRes.listingAttachment,
				new Object[] { "{promoId}", promoId, "{userId}", userId,
						"{skuId}", skuId, "{key}", key }));
		GingerWebTarget target = PromoClient.getClient().target(url);
		Invocation.Builder build = target.request();
		Response resp = build.headers(
				authHeaders(IAFTokenService.getIAFToken())).get();
		if (Status.OK.getStatusCode() == resp.getStatus()) {
			MultivaluedMap<String, Object> headers = resp.getMetadata();
			String attachmentName = (String) headers.get("attachmentName").get(
					0);
			if (attachmentName == null || "".equals(attachmentName)) {
				attachmentName = "download"; // default download file name
			}
			String attachmentType = (String) headers.get("attachmentType").get(
					0);
