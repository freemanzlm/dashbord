package com.ebay.raptor.promotion.nsd.service;


import java.util.HashMap;
import java.util.Map;

import org.apache.http.HttpException;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.ebay.app.raptor.cbtcommon.CommonLogger;
import com.ebay.app.raptor.cbtcommon.httpRequest.HttpRequestException;
import com.ebay.app.raptor.cbtcommon.httpRequest.HttpRequestService;
import com.ebay.app.raptor.cbtcommon.httpRequest.HttpResponseData;
import com.ebay.raptor.promotion.config.AppConfig;
import com.google.gson.reflect.TypeToken;

@Service
public class NSDDataService {
	private static final String POST_METHOD = "POST";
	private static final String GET_METHOD = "GET";
	private static final String NO_ENCRIPTION_PARAM = "?encrypt=false";
	private static String authorization;
	private static String nsdActGrade;
	private CommonLogger logger = CommonLogger.getInstance(NSDDataService.class);
	static {
		nsdActGrade = AppConfig.getSellerDashboardServicePrefix() + "/cbt/sdws2/v1/nsd/nsdactgrade/";
		
		if (AppConfig.bdCfg.isProduction()) {
			authorization = "IAF v^1.1#i^1#p^1#d^2015-05-27T22:24:47.778Z#r^0#I^2#f^0#t^";
		} else {
			authorization = "IAF v^1.1#i^1#I^2#f^0#p^1#d^2016-10-25T09:50:56.462Z#r^0#t^";
		}
	}

	@Autowired
	HttpRequestService httpRequestService;
	/**
	 * get the grade of sellers
	 * @param string
	 * @return
	 */
	public long getNSDActGradeByEbayOracleId(String ebayOracleId){   
		Map<String, String> headers = new HashMap<String, String>();
		headers.put("Authorization", authorization);
		try {
			String json = httpRequestService.doHttpRequest(nsdActGrade+ ebayOracleId,GET_METHOD, null, headers);
			TypeToken<HttpResponseData<Long>> type = new TypeToken<HttpResponseData<Long>>() {};
			HttpResponseData<Long> response = httpRequestService.getResponseData(json, type);
			if (response != null) {
				if (response.getIsSuccess()) {
					return response.getData();
				} else {
					logger.warn("Retrieving grade of seller from DB returns error, with error msg: "+ response.getErrorMmsg());
				}
			} else {
				logger.warn("Unable to get grade of seller.");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0;
	}
	
	
	
	public boolean  isUserAbleAccessNewBiz(Long userId){
		long useractGrade = getNSDActGradeByEbayOracleId(userId+"");
		if(useractGrade>0){
			return true;
		}
		return false;
	}
}
