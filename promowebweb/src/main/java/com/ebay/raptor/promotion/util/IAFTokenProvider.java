...
public class IAFTokenProvider {
	private static CommonLogger logger = CommonLogger.getInstance(IAFTokenProvider.class);
	private static String PRE = "CBT IAF - "; 
	
	static String CONSUMER_CLIENT = "IAFServiceClient";
	 ...
	 * @throws IAFException 
	 */
	public static String getTokenByConsumerIdAndSecret(String cid, String secret) {
		...
		String loginToken = getApplicationBasedToken(cid, secret);
		iafToken = getApplicationToken(cid, loginToken);
	...
	public static String geteTRSToken(String cid, String secret) {
		if(StringUtil.isEmpty(cid) || StringUtil.isEmpty(secret)){
			throw new IAFException("Consumer id or Secret is not valid, failed to get Token.");
		}
		String loginToken = getApplicationBasedToken(cid, secret);
		String appToken = getApplicationTokenWithoutScope(cid, loginToken);
	 ...
	 * @return
	 */
	private static String getApplicationTokenWithoutScope(String cid, String token) {
		String applicationToken = null;
		try {
			SharedIAFServiceConsumer defaultConsumer = new SharedIAFServiceConsumer(
					CONSUMER_CLIENT, getEnv());
			...
			if (rstResponse.getErrorMessage() != null) {
				...
				for (ErrorData e: eds) {
					logger.error(PRE + "Failed to fetch the application based token, error: " + e.getMessage());
				}
			} else {
			...
			logger.error(PRE + "Error happens when fetch the app based token. Error: " + e.getMessage(), e);
		}
	 ...
	 * @return
	 */
	private static String getApplicationToken(String cid, String token) {
		String applicationToken = null;
		try {
			SharedIAFServiceConsumer defaultConsumer = new SharedIAFServiceConsumer(
					CONSUMER_CLIENT, getEnv());
	 ...
	 * @return
	 */
	private static String getApplicationBasedToken(String cid, String secret) {
		...
		try {
			...
			synchronized(IAFTokenProvider.class){
				...
				logger.log(String.format("Start login IAF to get token for id %s. ", cid));
				response = iafServiceConsumer.login(request);
				logger.warn(String.format("Finish login IAF to get token for id %s time used %d.", cid, (System.currentTimeMillis() - start)));
			}
			if(null != response){
				if (response.getErrorMessage() != null) {
					List<ErrorData> eds = response.getErrorMessage().getError();
					for (ErrorData e : eds) {
						logger.error(PRE + "IAF login token fetch failed, error: " + e.getMessage() + " - " + e.toString());
					}
				} else {
					...
					if (stoken != null) {
						...
						logger.error(PRE + "IAF login token fetch returned empy value.");
					}
				}
			}
		} catch (ServiceException e) {
			logger.error(PRE + "IAF login token fetch failed. Error: " + e.getMessage(), e);
		}
		return logintoken;
	}
