...
<c:choose>
	<c:when test="${not empty subsidy}">
		<c:choose>
			<c:when test="${ subsidy.type eq 1 }">
				<c:set var="rewardName" value="加油卡" />
			</c:when>
			<c:when test="${ subsidy.type eq 2 }">
				<c:set var="rewardName" value="ebay万里通积分" />
			</c:when>
			<c:when test="${ subsidy.type eq 3 }">
				<c:set var="rewardName" value="万邑通礼品卡" />
			</c:when>
			<c:when test="${ subsidy.type eq 6 }">
				<c:set var="rewardName" value="京东卡" />
			</c:when>
			<c:when test="${ subsidy.type eq 4 }">
		...
		<c:choose>
			...
			<c:otherwise>
				<div class="promo-state-message success">
					<div class="message-content">
						<c:choose>
							<c:when	test="${ (subsidy.type eq 1 or subsidy.type eq 2 or subsidy.type eq 6) and subsidy.sellerRegion == 'CN'}">
				
								<h3>恭喜!您的奖励为等值&nbsp;${subsidy.amount} ${subsidy.currency}的${rewardName }</h3>
				
								<c:choose>
									<c:when test="${ state eq 'SubsidyRetrievable' }">
										<c:if test="${ subsidy.type eq 1}">
											<div class="note">
							...
							<c:when test="${ subsidy.type eq 3 }">
								<h3>恭喜!您的奖励为等值&nbsp;${reward} ${subsidy.currency}的${rewardName }</h3>
					...
					<c:choose>
						<c:when
							test="${ (((promo.subsidy.type eq 1 or subsidy.type eq 2) and subsidy.sellerRegion == 'CN') or subsidy.type eq 3)
							...
							<menu>
								<li>
									<c:choose>
										...
										<c:when test="${ state eq 'SubsidyRetrievable' }">
											<c:choose>
												<c:when test="${ subsidy.type eq 2 and subsidy.sellerRegion == 'CN'}">
